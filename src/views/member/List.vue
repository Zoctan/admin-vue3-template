<template>
  <div class="app-container">
    ...
    >
        ...
        <el-form-item label="Username" prop="member.username">
          ...
          >
            <template #append>
              <el-button
                icon="refresh-right"
                @click="getFakeUsername"
                :loading="getFakeUsernameLoading"
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="danger" @click="resetForm(memberFormRef)"
            >Reset</el-button
          >
          <el-button
            type="primary"
            ...
            @click="
              dialogMemberStatusMap[dialogMemberStatus].submitAction(
                memberFormRef
              )
            "
            >Confirm</el-button
    ...
    <el-dialog
      v-model="dialogUpdateMemberRoleVisible"
    ...
    >
      ...
      >
        <el-form-item label="Current Role">
          <template
            v-for="(role, index) in memberRoleForm.roleList"
            :key="role.id"
          >
            <el-tag
              size="small"
              ...
              class="mr-1"
              closable
              @close="onDeleteMemberRole(index)"
              >{{ role.name }}</el-tag
            >
          </template>
        </el-form-item>
...
import {
  list as listMember,
  detail as getMemberDetail,
  updateDetail as updateMemberDetail,
  add as addMember,
...
const getFakeUsername = () => {
  ...
  getFakeName()
    ...
    .catch((error) => {
      ElMessage.error(`get fake username error: ${JSON.stringify(error)}`);
    })
    .finally(() => {
...
const onDeleteMemberRole = (index) => {
  ...
  deleteMemberRole({ roleId: roleId, memberId: memberRoleForm.memberId })
    .then(() => {
