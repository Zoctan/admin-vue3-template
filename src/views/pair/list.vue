  ...
  <div class="app-container">
      ...
      <el-form :inline="true" ref="searchFormRef" :model="searchForm">
        <el-form-item>
          ...
          <el-button
            ...
            icon="plus"
            circle
            @click="showAddPairDialog"
          ></el-button>
        </el-form-item>
        <el-form-item label="Description" prop="pair.description">
          <el-input v-model="searchForm.pair.description"></el-input>
        </el-form-item>
        <el-form-item label="Key" prop="pair.key">
          <el-input v-model="searchForm.pair.key"></el-input>
        </el-form-item>
        <el-form-item label="Value" prop="pair.value">
          <el-input v-model="searchForm.pair.value"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button
            type="primary"
    ...
    >
      ...
      <el-table-column
        fixed="right"
        label="Operations"
        v-permission:or="['pair:update', 'pair:delete']"
      >
        <template #default="scope">
          <el-space wrap>
            <span v-permission="'pair:update'">
              <el-button @click="showUpdatePairDialog(scope.row.id)"
                >Update</el-button
              >
            </span>
            <span v-permission="'pair:delete'">
              <el-popconfirm
                confirm-button-text="Yes"
                cancel-button-text="No"
                icon-color="red"
                :title="`Are you sure to delete this pair?`"
    ...
    <el-dialog
      v-model="dialogPairVisible"
      :title="dialogPairStatusMap[dialogPairStatus].title"
      destroy-on-close
    >
      <el-form
        ref="pairFormRef"
      ...
      >
        <el-form-item label="Description" prop="pair.description" required>
          <el-input
            type="text"
            autocomplete="off"
            v-model="pairForm.pair.description"
          />
        </el-form-item>
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="danger" @click="resetForm(pairFormRef)"
            >Reset</el-button
          >
          <el-button
            type="primary"
            :loading="submitPairLoading"
            :disabled="submitPairDisabled"
            @click="
              dialogPairStatusMap[dialogPairStatus].submitAction(pairFormRef)
            "
            >Confirm</el-button
          >
        </span>
...
import {
  list as listPair,
  add as addPair,
  update as updatePair,
  remove as removePair,
} from "api/pair";
...
const getPairList = (finalCallback = null) => {
  ...
  listPair(searchForm)
    ...
    .catch((error) => {
      ElMessage.error(`get pairList error: ${JSON.stringify(error)}`);
    })
    .finally(() => {
...
const onAddPair = () => {
  ...
  pairForm.pair.value = jsonEditor.get();
  addPair(pairForm)
    .then(() => {
      getPairList();
    ...
    .catch((error) => {
      ElMessage.error(`add pair error: ${JSON.stringify(error)}`);
    })
    .finally(() => {
...
const showUpdatePairDialog = (pairId) => {
  ...
  for (let i = 0; i < pairList.value.length; i++) {
    if (pairList.value[i].id === pairId) {
      ...
      jsonEditor.set(pairList.value[i].value);
...
const onUpdatePair = () => {
  ...
  updatePair(pairForm)
    ...
    .catch((error) => {
      ElMessage.error(`update pair error: ${JSON.stringify(error)}`);
    })
    .finally(() => {
      submitPairLoading.value = false;
      submitPairDisabled.value = false;
